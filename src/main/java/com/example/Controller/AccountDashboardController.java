...
public class AccountDashboardController implements ViewLoader {
    ...
    void btnCreateAccountClicked(ActionEvent event) {
        ...
        try {
            if (accountNumber.isEmpty() || accountTypeStr == null) {
                LogToFile.logToFile("WARNING", "Usuario " + UserService.getCurrentUser().getName() + " intentó crear una cuenta con campos vacíos.");
                throw new EmptyFieldException("Por favor, complete todos los campos.");
            }

            if (!accountNumber.matches("\\d{10,16}")) {
                LogToFile.logToFile("WARNING", "Usuario " + UserService.getCurrentUser().getName() + " ingresó un número de cuenta inválido.");
            ...
            try {
                ...
                LogToFile.logToFile("WARNING", "Usuario " + UserService.getCurrentUser().getName() + " ingresó un número de cuenta no numérico.");
            ...
            if (AccountService.doesAccountExist(accountNumber)) {
                LogToFile.logToFile("WARNING", "Usuario " + UserService.getCurrentUser().getName() + " intentó crear una cuenta duplicada.");
            ...
            LogToFile.logToFile("INFO", "Usuario " + UserService.getCurrentUser().getName() + " creó la cuenta con número " + accountNumber + " exitosamente.");
            ...
            LogToFile.logToFile("ERROR", "Error durante la creación de cuenta para el usuario " + UserService.getCurrentUser().getName() + ": " + e.getMessage());
    ...
    void btnCancelClicked(ActionEvent event) {
        LogToFile.logToFile("INFO", "Usuario " + UserService.getCurrentUser().getName() + " canceló la creación de una cuenta.");
        tfAccountNumber.clear();
        cbAccountType.setValue(null);
        lblMessage.setText("");
